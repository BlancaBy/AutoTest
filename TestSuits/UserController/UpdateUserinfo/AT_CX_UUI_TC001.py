...
filepath = project_dir + '/Data/UserController/UpdateUserinfo/TestData.xlsx'
caseId = 0
class TC001(unittest.TestCase):
    def setUp(self):
        ...
        self.updateUserinfo = UpdateUserinfo(self.dictdata[caseId])
    def testCase(self):
        '''更新用户信息'''
        print '-' * 20, 'AT_CX_UUI_TC001：更新用户信息', '-' * 20
        ...
        r = self.updateUserinfo.send_request(request)
        print r.text
        self.updateUserinfo.get_status(r)
        print "--------------------->"
        # endregion

        # region step2：断言
        result = self.updateUserinfo.verify_success_return(r)
        # 断言结果写入到excel中的result列
        pfAPI.write_result_in_csv(filepath, caseId + 2, r, result)
        pfAPI.assert_condition('AT_CX_UUI_TC001', '：更改用户信息', result)
        # endregion
        # endregion
