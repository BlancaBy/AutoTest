...
from APIS.CustomMessageController.ForwardMessage import ForwardMessage
...
filepath = project_dir + '/Data/CustomMessageController/ForwardMessage/TestData.xlsx'
caseId = 0
class TC001(unittest.TestCase):
    def setUp(self):
        ...
        self.forwardMessage = ForwardMessage(self.dictdata[caseId])
    def testCase(self):
        '''接人推送自定义消息回执'''
        print '-' * 20, 'AT_CX_FM_TC001：转发自定义消息回执', '-' * 20
        ...
        r = self.forwardMessage.send_request(request)
        print r.text
        self.forwardMessage.get_status(r)
        print "--------------------->"
        # endregion

        # region step2：断言
        result = self.forwardMessage.verify_success_return(r)
        # 断言结果写入到excel中的result列
        pfAPI.write_result_in_csv(filepath, caseId + 2, r, result)
        pfAPI.assert_condition('AT_CX_FM_TC001', '：转发自定义消息回执', result)
        # endregion
        # endregion
