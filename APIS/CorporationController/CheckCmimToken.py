...
class CheckCmimToken():
    ...
    # region 业务操作
    # 发送request
    def send_request(self, thirdPartyToken,corporationSerial="001"):
        self.cmimToken=thirdPartyToken
        #
        self.config['url']=self.config['url'].replace("{cmimToken}",thirdPartyToken)
        self.config['url']=self.config['url'].replace("{corporationSerial}",corporationSerial)
