...
class SdkSendMessages():
    '''
    用户登录
        Method：POST
        URL：https://<base url>/user/v0/sdkSendMessages
        参数：
        {
        "data": {},
        "messType": 0,
    ...
    def verify_success_return(self, r):
        ...
        result = (bool(status == "0000_0") or bool(status == "0002_3")) and \
                 bool(statusCode == 200) and (bool(message == '执行成功')or bool(message == '您已超出每天10条接人短信上限'))
        return result
