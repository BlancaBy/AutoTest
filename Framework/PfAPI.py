...
def sendRequests(method, url, **kwargs):
    ...
    # 从excel取出用户信息
    if (string.find(url,"ogin")) == -1:
        row = pfFile.read_data_from_csv(project_dir+"\Data\LoginController\Login\TestData.xlsx")[0]
        row1 = pfFile.read_data_from_csv(project_dir + "\Data\CorporationController\CorporationLogin\TestData.xlsx")[0]
        result = json.loads(row["Response"])["result"]
        result1 = json.loads(row1["Response"])["result"]

    #取出token
        kwargs["headers"]["cmimToken"] = result["token"]#取用户token
        kwargs["headers"]["corporationToken"] = result1["token"]#取企业token
        print 'token值:', result["token"]
    print '访问地址：', url
